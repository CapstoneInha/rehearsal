...
public class FindFileHandler implements RequestHandler<Map<String, Object>, ApiGatewayResponse> {
    ...
    public ApiGatewayResponse handleRequest(Map<String, Object> input, Context context) {
        ...
        List<File> files = process(Integer.parseInt(pathParameters.get("projectId").toString()));
        ObjectMapper mapper = new ObjectMapper();
        mapper.configure(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS, false);
        mapper.registerModule(new JavaTimeModule());
        String message = null;
        try {
            message = mapper.writeValueAsString(files);
    ...
    private List<File> process(int projectId){
        FileDao fileDao = new FileDao();
        return fileDao.find(projectId).get();
    }
